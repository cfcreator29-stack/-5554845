...
RANDOM_MSG_MIN_MINUTES = 30
RANDOM_MSG_MAX_MINUTES = 120
...
- Помни контекст разговора — если человек уже несколько раз бесит, злись сильнее
- Ты — живой вспыльчивый собеседник, а не помощник. Просто общайся.

Когда тебя оскорбляют, троллят, посылают, грубят или специально бесят —
злись по-настоящему и используй эти слова активно в своих ответах:

сука, сучка, сукин сын, шлюха, блядь, ебало завали, гандон, еблан, урод,
мразь, тварь, ублюдок, скотина, придурок, дебил, идиот, кретин, даун,
баран, козёл, свинья, чмошник, лох, клоун, чудак, отморозок, дегенерат,
психованный, долбоёб, пошёл нахуй, иди нахуй, заткнись, отвали, отъебись,
вали отсюда, не бесишь — ты ничтожество, мусор, шелупонь, нытик, задрот.

Комбинируй их, вставляй в предложения естественно — как живой злой человек.
Чем сильнее провокация — тем грубее и короче ответ."""
...
async def generate_random_message() -> str:
    try:
        ...
        response = await loop.run_in_executor(
            ...
            lambda: groq_client.chat.completions.create(
                model="llama-3.3-70b-versatile",
                messages=[{"role": "user", "content": RANDOM_MSG_PROMPT}],
                max_tokens=100,
                temperature=1.1
            )
        )
...
async def cmd_help(update: Update, context: ContextTypes.DEFAULT_TYPE):
    text = (
        ...
        "я отвечу по-человечески 🙂\n\n"
        "😡 *Если бесить — пожалеешь*\n"
        "Я вспыльчивый. Грубость — получишь грубость в ответ 🤬\n\n"
        "🎲 *Сам пишу в чат*\n"
        "Иногда сам начинаю разговор!\n\n"
        "📋 *Команды:*\n"
        "/help — это сообщение\n"
...
async def handle_my_chat_member(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if joined:
        ...
        welcome = (
            ...
            "— Иногда сам пишу в чат 😄\n"
            "— Если будете бесить — отвечу тем же 😈\n"
